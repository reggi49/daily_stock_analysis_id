    ...
    if target_enabled:
        if password or confirm:
            if stored_password_exists:
                return JSONResponse(
                    ...
                    content={
                        ...
                        "message": "Admin password already exists, enable authentication first then use the change password feature to update",
                    },
                )
            if not password:
                return JSONResponse(
            ...
            if password != confirm:
                return JSONResponse(
                    status_code=400,
                    content={"error": "password_mismatch", "message": "Passwords entered twice do not match"},
                )
            ...
            return JSONResponse(
                status_code=400,
                content={"error": "password_required", "message": "Please set a password before enabling password login"},
            )
        else:
            ...
            if not is_valid_session:
                if not current_password:
                    return JSONResponse(
                        status_code=400,
                        content={"error": "current_required", "message": "Please enter current password before re-enabling authentication"},
                    )
        ...
        if current_enabled:
            ...
            if not is_valid_session:
                if not current_password:
                    return JSONResponse(
                        status_code=400,
                        content={"error": "current_required", "message": "Please enter current password before disabling authentication"},
                    )
                ip = get_client_ip(request)
                if not check_rate_limit(ip):
                    return JSONResponse(
...
async def auth_login(request: Request, body: LoginRequest):
    ...
    if not password:
        return JSONResponse(
            status_code=400,
            content={"error": "password_required", "message": "Please enter password"},
        )
    ...
    if not password_set:
        ...
        if not verify_password(password):
            ...
            return JSONResponse(
                status_code=401,
                content={"error": "invalid_password", "message": "Password is incorrect"},
            )
...
async def auth_change_password(body: ChangePasswordRequest):
    ...
    if not current:
        return JSONResponse(
            status_code=400,
            content={"error": "current_required", "message": "Please enter current password"},
        )
    if new_pwd != new_confirm:
        return JSONResponse(
            status_code=400,
            content={"error": "password_mismatch", "message": "New passwords entered twice do not match"},
        )
