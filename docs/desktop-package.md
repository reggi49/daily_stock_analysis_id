...
echo \"Should check if the attachment contains:\"
...
- 如缺少上述文件或 `release-tag` 不匹配，需标注阻断并补齐 `desktop-release` Packaging flow。
...
- 从旧版本升级时，如果旧 `.app` 包内部的 `.env`、`data/stock_analysis.db` 或日志文件仍可访问，新版本会在目标文件不存在时自动Migrate to用户数据目录；已有目标文件不会被覆盖
- 用户需要编辑 `.env` 配置以下内容：
...
- 导入是“Key-level override”而不是整文件替换：备份文件中出现的键会覆盖当前值，未出现的键保持不变
...
- `README.md` 继续保留安装和运行入口说明；这类桌面端运行时细节统一落在本Special documentation维护，避免入门文档膨胀

### 局域网访问 Windows 桌面端 WebUI

- 桌面端默认仍按 `WEBUI_HOST=127.0.0.1` Only allow local access，避免安装后无意暴露后端服务
