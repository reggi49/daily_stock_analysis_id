...
Extract stock codes from screenshots/images using Vision LLM.
...
EXTRACT_PROMPT = """Please analyze this stock market screenshot or image and extract all visible stock codes and names.

Important: If both stock names and codes are shown simultaneously (e.g. watchlist, ETF list), you must extract both, and each element must contain code and name fields.
...
- name: required when visible in the image (e.g. Kweichow Moutai, Bank ETF, Securities ETF), matching one-to-one with code; may be omitted only if no name is visible
- confidence: required, recognition confidence, high=certain, medium=relatively certain, low=uncertain

Examples (when both name and code are shown):
- A-share: 600519 Kweichow Moutai, 300750 CATL
- HK: 00700 Tencent, 09988 Alibaba
- US: AAPL Apple, TSLA Tesla
- ETF: 159887 Bank ETF, 512880 Securities ETF, 512000 Broker ETF, 512480 Semiconductor ETF, 515030 New Energy Vehicle ETF

Output example: [{"code":"600519","name":"Kweichow Moutai","confidence":"high"},{"code":"159887","name":"Bank ETF","confidence":"high"}]

Do NOT return code-only arrays like ["159887","512880"]; object format is required. If no stock codes are found, return: []"""
...
def _verify_image_magic_bytes(image_bytes: bytes, mime_type: str) -> None:
    ...
    if mime_type not in _IMAGE_SIGNATURES:
        raise ValueError(f"Cannot verify type: {mime_type}")
    if mime_type == "image/webp":
        if image_bytes[:4] != b"RIFF" or image_bytes[8:12] != b"WEBP":
            raise ValueError("File content does not match declared type image/webp; possible tampering")
        return
    for sig in _IMAGE_SIGNATURES[mime_type]:
        if image_bytes.startswith(sig):
            return
    raise ValueError(f"File content does not match declared type {mime_type}; possible tampering")
...
def _parse_codes_from_text(text: str) -> List[str]:
    ...
    # Prefer trying JSON array; only strip leading markdown fence, avoid rfind("```") accidentally removing trailing content
    cleaned = text.strip()
    for start in ("```json", "```"):
    ...
    # Fallback: find 5-6 digit codes and US stock codes
...
def _parse_items_from_text(text: str) -> List[Tuple[str, Optional[str], str]]:
    ...
    if not codes:
        logger.info("[ImageExtractor] Unable to parse as structured items, and legacy code extraction returned empty")
...
def _call_litellm_vision(image_b64: str, mime_type: str, api_key: Optional[str] = None) -> str:
    ...
    if not model:
        raise ValueError("Vision API not configured. Please set LITELLM_MODEL or related API Key.")
    if route_has_hermes(getattr(cfg, "llm_model_list", []) or [], model):
        raise ValueError("Hermes Vision not verified: VISION_MODEL cannot select routes containing Hermes deployments.")
...
def extract_stock_codes_from_image(
    ...
    """
    Extract stock codes and names from image (using Vision LLM).

    Priority: Gemini -> Anthropic -> OpenAI (first available).
    Supports multi-key rotation and retry (up to 3 attempts, exponential backoff).
    ...
    Returns:
        (items, raw_text) - items is [(code, name?, confidence), ...], raw_text is the raw LLM response.

    Raises:
        ValueError: When image is invalid, Vision API is not configured, or extraction fails.
    """
    ...
    raise ValueError(
        f"Vision API call failed, please check API Key and network: {last_error}"
    ) from last_error
