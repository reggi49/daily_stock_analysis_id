...
_STRONG_COMPARE_PATTERN = re.compile(r"compare|Contrast|vs\b|and[^，。,.!?！？]{0,40}Than", re.IGNORECASE)
_WEAK_COMPARE_HINT_PATTERN = re.compile(r"difference(?!ization)|difference|different|compared to|control|Compare")
_CHOICE_COMPARE_PATTERN = re.compile(r"Which|Which one|Which one|Who is more|more worthy|more suitable|How to choose|Which to choose|Choose one")
_LINKED_COMPARE_PATTERN = re.compile(
    r"(?:and|with|follow|Same)(?P<body>[^，。,.!?！？]{0,40})(?:difference(?!ization)|difference|different|compared to|control|Compare)"
)
_SWITCH_PATTERN = re.compile(r"Replace with|Watch it instead|analysis|take a look|Research|Diagnosis")
...
_INDICATOR_CONTEXT_PATTERN = re.compile(
    r"indicator|moving average|moving average|arrange|Long|short|golden fork|Sicha|support|pressure|MA\d|SMA|EMA",
    re.IGNORECASE,
)
