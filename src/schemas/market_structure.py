...
class MarketStructureSource(BaseModel):
    provider: str = Field(..., description="Data source identifier, only for snapshot metadata, does not participate in runtime provider/model routing")
    dataset: str = Field(..., description="Dataset identifier, only for historical traceable snapshots")
    status: str = Field("ok", description="Source availability snapshot")
    message: Optional[str] = Field(None, description="Source message, only for display/troubleshooting")
