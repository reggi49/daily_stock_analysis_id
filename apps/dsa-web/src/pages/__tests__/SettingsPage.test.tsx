...
const baseCategories = [
  ...
  { category: 'base', title: 'Base', description: 'Basic config', displayOrder: 2, fields: [] },
  { category: 'ai_model', title: 'AI', description: 'Model config', displayOrder: 3, fields: [] },
  { category: 'notification', title: 'Notification', description: 'Notification config', displayOrder: 4, fields: [] },
...
describe('SettingsPage', () => {
    ...
    getSetupStatus.mockResolvedValue({
      ...
      checks: [
        ...
        },
        {
          ...
          required: true,
          status: 'configured',
          message: 'Model channels configured.',
          nextStep: null,
        },
        {
          key: 'notification',
  ...
  it('renders category navigation and auth settings modules', async () => {
    ...
    expect(screen.getByText('Auth and login protection')).toBeInTheDocument();
  ...
  it('keeps first-run setup summary neutral while setup status is loading', async () => {
    ...
    expect(screen.getByText('Reading config status. Missing items and trial run will appear when ready.')).toBeInTheDocument();
    expect(screen.queryByText('Basic config meets minimum viable analysis')).not.toBeInTheDocument();
    expect(screen.queryByText('Basic configuration still needs attention')).not.toBeInTheDocument();
    expect(screen.queryByText('All required items ready. Run a quick analysis to verify the pipeline.')).not.toBeInTheDocument();
  });
  ...
  it('keeps first-run setup summary neutral when setup status fails', async () => {
    ...
    expect(await screen.findByText('Unable to determine config status')).toBeInTheDocument();
    expect(screen.getByText('Failed to read config status. Check or modify settings, then refresh.')).toBeInTheDocument();
    expect(screen.queryByText('Basic config meets minimum viable analysis')).not.toBeInTheDocument();
    expect(screen.queryByText('Basic configuration still needs attention')).not.toBeInTheDocument();
    expect(screen.queryByText('All required items ready. Run a quick analysis to verify the pipeline.')).not.toBeInTheDocument();
  });
  ...
  it('keeps the latest first-run setup status when refresh responses resolve out of order', async () => {
      ...
      checks: [
        {
          ...
          required: true,
          status: 'needs_action',
          message: 'Stale config status.',
          nextStep: 'This stale response should not override the latest status.',
        },
      ],
    };
    ...
    expect(screen.queryByText('Stale status')).not.toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Quick trial run' })).toBeEnabled();
  });
  ...
  it('runs a brief setup smoke analysis with the first watchlist stock', async () => {
    ...
    await screen.findByText('Basic config meets minimum viable analysis');
    fireEvent.click(screen.getByRole('button', { name: 'Quick trial run' }));
  ...
  it('allows brief setup smoke when only the Agent channel is incomplete', async () => {
    ...
    getSetupStatus.mockResolvedValue({
      ...
      checks: [
        ...
        },
        {
          ...
          required: true,
          status: 'needs_action',
          message: 'Agent tool calling requires LiteLLM model config; local CLI primary generation is not auto-inherited.',
    ...
    await screen.findByText('1 item missing: Agent channel');
    expect(screen.getByRole('button', { name: 'Quick trial run' })).toBeEnabled();
  ...
  it('shows missing setup items and lets the user reopen the setup check', async () => {
      ...
      checks: [
        {
          ...
          required: true,
          status: 'needs_action',
          message: 'Model channels not configured yet.',
          nextStep: 'Please configure model channels first.',
        },
      ],
    });
    useSystemConfigMock.mockReturnValue(buildSystemConfigState({ activeCategory: 'base' }));
    ...
    expect(await screen.findByText('Basic configuration still needs attention')).toBeInTheDocument();
    expect(screen.getByText('1 item missing: Model channels')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Quick trial run' })).toBeDisabled();

    fireEvent.click(screen.getByRole('button', { name: 'Dismiss' }));
    expect(screen.getByText('First-run setup check dismissed')).toBeInTheDocument();

    fireEvent.click(screen.getByRole('button', { name: 'Expand check' }));
  ...
  it('normalizes malformed desktop update payloads instead of throwing', async () => {
    ...
    expect(screen.queryByText('New version found')).not.toBeInTheDocument();
  });
  ...
  it('renders env backup actions outside desktop runtime', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Config backup' })).toBeInTheDocument();
  ...
  it('asks for confirmation before importing when local drafts exist', async () => {
    ...
    expect(await screen.findByText('Import will overwrite current draft')).toBeInTheDocument();
  ...
  it('renders desktop update notice when a newer release is available', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Go to download' })).toBeInTheDocument();
  });
