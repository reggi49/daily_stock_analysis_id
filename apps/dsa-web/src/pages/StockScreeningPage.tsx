...
const MARKETS = [{ id: 'cn', label: A-shares }];
...
const isUnrecoverableScreenTaskError = (error: ParsedApiError) =>
  error.title === 'Screening task not recoverable';
...
const formatRecoverableScreenTaskPollingError = (error: ParsedApiError) => {
  ...
  if (error.category === 'upstream_network' || error.category === 'local_connection_failed') {
    return 'The screening task is still running in the background; cannot reach the local service for status and will retry automatically.';
  }
  return formatParsedApiError(error) || 'Cannot get the screening task status for now; will retry automatically later.';
};
...
const getCandidateReason = (item: AlphaSiftCandidate) => {
  ...
  return 'AlphaSift returned candidates but no text summary. See the factors, risks, and raw fields below.';
};
...
const summarizeAlphaSiftDiagnostic = (detail: string) => {
  if (/trade_cal returned no open trading days/i.test(detail)) {
    return 'No trading day available in the calendar';
  }
  ...
  if (/missing .*api key|GEMINI_API_KEY|GOOGLE_API_KEY|gemini_api_key/i.test(detail)) {
    return 'Missing a usable LLM API key';
  }
...
const formatHotspotEmptyMessage = (result: AlphaSiftHotspotsResponse) => {
  ...
  if (message === ALPHASIFT_HOTSPOT_NO_CACHE_HINT) {
    return 'No cached hot themes yet. Expand and click refresh to fetch live data.';
  }
...
const getHotspotStrength = (item: AlphaSiftHotspot, index: number) => {
  ...
  if (index === 0 || heat >= 90 || changePct >= 8) {
    return { label: 'Leading strength', className: 'bg-red-500/10 text-red-500' };
  }
  if (heat >= 80 || changePct >= 5) {
    return { label: 'Strong', className: 'bg-blue-500/10 text-blue-500' };
  }
  return { label: 'Moderate', className: 'bg-cyan/10 text-cyan' };
};
...
const StockScreeningPage: React.FC = () => {
  ...
  const alertMessages = llmDegraded
    ? screenMessages.length > 0
      ? screenMessages
      : ['LLM rerank did not complete or returned no judgment; current candidates come from AlphaSift local factor scoring.']
    : screenMessages;
    ...
    function applyTaskStatus(task: AlphaSiftScreenTaskStatus) {
      ...
      if (task.status === 'completed') {
        if (task.result) {
          ...
          setError('The screening task completed, but the server returned no candidate results.');
          setCandidates([]);
          setScreenMeta(null);
        }
        finishTask();
      ...
      setError(`Screening task returned an unknown status: ${task.status || 'unknown'}`);
      finishTask();
    }
    ...
    async function pollTask() {
      try {
        ...
        if (isUnrecoverableScreenTaskError(parsedError)) {
          setError(formatParsedApiError(parsedError) || 'Screening task not recoverable. Please resubmit.');
          setCandidates([]);
          setScreenMeta(null);
          finishTask();
          return;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setCandidates([]);
      setLoading(false);
      setError(toApiErrorMessage(err, 'Screening task submission failed. Please try again later.'));
    }
  };
  ...
  return (
    <AppPage className="max-w-6xl space-y-6 pb-12 pt-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-secondary-text">When enabled, generates candidate stocks via the built-in AlphaSift adapter and supplements them with DSA data and news</p>
          </div>
        </div>
      ...
      {!enabled ? (
        <InlineAlert
          variant="info"
          title="AlphaSift not enabled"
          message="Clicking writes ALPHASIFT_ENABLED=true. AlphaSift ships with backend dependencies; if the adapter layer is missing, update dependencies or rebuild the backend first."
          action={
      ...
      {enabled && !available ? (
        <InlineAlert
          ...
          message="The adapter layer is currently unavailable. Confirm the backend dependencies are installed and restart the service. If needed, run pip install -r requirements.txt or use the settings page / server /install endpoint to repair-install."
        />
      ) : null}
      ...
      <InlineAlert
        ...
        message="AlphaSift screening is still experimental. Results are for research and reference only and do not constitute investment advice. Markets carry risk; trading decisions and gains or losses are the user's own responsibility."
      />
      ...
      <section className="rounded-2xl border border-border/80 bg-card/95 p-4 shadow-soft-card">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
               ...
               <p className="mt-1 text-xs leading-5 text-secondary-text">
                 From AlphaSift's latest hotspot capability; strategies like capital_heat and balanced_alpha include theme_heat in scoring.
               </p>
            </div>
          </div>
        ...
        {!hotspotsExpanded ? (
          <div className="flex flex-col gap-2 rounded-xl border border-border/70 bg-surface/70 px-4 py-3 text-sm text-secondary-text sm:flex-row sm:items-center sm:justify-between">
            <span>
              {hotspots.length > 0
                ? `Cached ${hotspots.length} hot themes. Expand to view heat, phase, and momentum path.`
                : 'Hot themes are collapsed by default. Expand to read the cache; click refresh to fetch live data.'}
            </span>
            <span className="text-xs">Live details load after you select a specific theme</span>
          </div>
        ) : hotspots.length === 0 ? (
          <div className="rounded-xl border border-dashed border-border bg-surface/70 px-4 py-6 text-sm text-secondary-text">
            Clicking refresh pulls hot concept/industry rankings, heat scores, lifecycle phase, and active leaders.
          </div>
        ) : (
            ...
            {hotspots.map((item, index) => {
              ...
              >
                ...
                <div className="mt-4 grid max-w-[72%] gap-1 text-[11px] text-secondary-text">
                  ...
                  <span>Trend <strong className="font-semibold text-foreground">{formatHotspotMetric(item.trendScore)}</strong> · Duration <strong className="font-semibold text-foreground">{formatHotspotMetric(item.persistenceScore)}</strong></span>
                  <span>{getHotspotSampleText(item)} · Leader {getHotspotLeadersText(item)}</span>
                </div>
          ...
          <div className="mt-4 rounded-xl border border-border/80 bg-surface/80 p-4">
            <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-secondary-text">
                  {loadingHotspotDetail ? 'Reading momentum path and concept stocks...' : hotspotDetail?.summary || 'Click a theme to view its momentum path and concept stocks.'}
                </p>
            ...
            {hotspotDetail && ((hotspotDetail.missingFields || []).length > 0 || (hotspotDetail.sourceErrors || []).length > 0) ? (
              <details className="mb-3 rounded-xl border border-warning/30 bg-warning/10 px-3 py-2 text-xs text-warning">
                 <summary className="cursor-pointer font-semibold">Detail data degraded, expand to see why</summary>
      ...
      <section className="rounded-2xl border border-cyan/35 bg-card/95 p-4 shadow-soft-card">
        ...
        <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-4">
          {loadingStrategies ? (
            ...
            <div className="rounded-xl border border-dashed border-border bg-surface/70 p-4 text-sm text-secondary-text">
              {strategyLoadError || 'AlphaSift strategy list not loaded yet. You can enter strategy parameters manually below.'}
            </div>
          ) : (
            strategies.map((item) => {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-semibold text-foreground">
                {loading ? 'Screening...' : candidates.length > 0 ? 'Screening complete' : isScreeningEnabled ? 'Waiting to run' : 'Waiting to enable'}
              </h2>
      ...
      {screenMeta && alertMessages.length > 0 ? (
        <InlineAlert
          ...
          title={llmDegraded ? 'LLM degraded' : 'AlphaSift hint'}
