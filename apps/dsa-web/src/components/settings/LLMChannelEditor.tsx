...
const RUNTIME_CAPABILITY_OPTIONS: Array<{ value: LLMCapabilityCheck; label: string; hint: string }> = [
  { value: 'json', label: 'JSON', hint: 'Check if response_format JSON output is available.' },
  { value: 'tools', label: 'Tools', hint: 'Check if function/tool calling is available.' },
  { value: 'stream', label: 'Stream', hint: 'Check if streaming output can return valid chunks.' },
  { value: 'vision', label: 'Vision', hint: 'Check if the current model accepts image_url input.' },
];
...
const RUNTIME_ONLY_HERMES_SECRET_MESSAGE = 'Runtime-injected Hermes Key will not be sent back; to test in settings, re-enter the Key or save it to .env.';
      ...
      {expanded ? (
        <div className="settings-surface-overlay-soft space-y-4 px-4 py-4">
          ...
          <Input
            ...
            placeholder={
              channel.protocol === 'gemini' || channel.protocol === 'anthropic'
                ? 'Official endpoint, can be left empty'
          ...
          {showProviderTemplateDetails ? (
            <div className="space-y-2 rounded-xl border border-[var(--settings-border)] bg-[var(--settings-surface-hover)] p-3">
              ...
              <p className="text-[11px] leading-5 text-muted-text">
                Capability labels are for configuration reference only and do not indicate runtime capability verification.
              </p>
            </div>
          ) : null}
          ...
          <Input
            ...
            placeholder={channel.protocol === 'ollama' ? 'Local Ollama, can be left empty' : 'Supports multiple keys, comma-separated'}
          />
          </div>
          ...
          <div className="space-y-3 rounded-xl border border-[var(--settings-border)] bg-[var(--settings-surface-hover)] p-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {discoveryState?.text || 'OpenAI Compatible channels that support `/models` can auto-discover models.'}
              </span>
            </div>
            {discoveryState?.hint ? (
            ...
            <Input
              ...
              hint={
                discoveredModels.length > 0
                  ? 'If custom model names not in the list, you can add them manually. Save format is still comma-separated.'
                  : 'If the channel does not support auto-discovery or the request fails, you can fill in the model list manually.'
              }
            />
            </div>
          ...
          <div className="flex items-center gap-2 pt-1">
            ...
            {testState?.text ? (
              <div className="space-y-1">
                ...
                {selectedModels[0] ? (
                  <p className="text-[11px] text-secondary-text">
                    Basic connection test uses the first model in the list by default: {selectedModels[0]}
                  </p>
                ) : null}
                {testState.hint ? (
                  <p className="text-[11px] text-secondary-text">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <p className="mt-0.5 text-[11px] text-secondary-text">
                  Real LLM requests are only sent when manually triggered; multi-select may take 20-40 seconds.
                </p>
              </div>
              <Button
                type="button"
...
const LLM_TROUBLESHOOTING_HINTS: Record<string, string> = {
  auth: 'Check if the API Key is correct, has no extra spaces, and whether the channel requires additional organization/project permissions.',
  timeout: 'Can retry; if persistent, check Base URL, network proxy, provider availability zone, or local firewall.',
  quota: 'Check balance, plan quota, RPM/TPM rate limits, or concurrency settings; retry later if needed.',
  model_not_found: 'Verify the model name matches the channel protocol, and use "Fetch Models" to check the actual available model list for this channel.',
  empty_response: 'Channel connected but returned no content; try switching to a compatible model or disabling extra response modes before testing.',
  network_error: 'Check Base URL, proxy, TLS/certificate, transit gateway, or local network policy; retry later.',
  invalid_config: 'Complete the protocol, Base URL, API Key, and model configuration before running the connection test.',
  unsupported_protocol: 'Only OpenAI Compatible / DeepSeek channels support auto model discovery; please maintain the model list manually.',
};

const LLM_REASON_HINTS: Record<string, string> = {
  missing_api_key: 'API Key is empty or no valid key after comma splitting; enter at least one valid key before testing.',
  api_key_rejected: 'Provider rejected the current API Key; check the key, organization/project permissions, region, and account status.',
  rate_limit: 'Provider triggered RPM/TPM or concurrency rate limiting; reduce request frequency or retry later.',
  insufficient_balance: 'Provider returned insufficient balance, billing, or quota; check account balance and plan status.',
  quota_exceeded: 'Provider returned quota exhausted; verify account plan, remaining quota, and project limits.',
  provider_blocked: 'Request blocked by provider or transit gateway; check account risk controls, region restrictions, model permissions, reseller gateway policies, content safety policies, or request origin restrictions.',
  dns_error: 'DNS resolution failed; check Base URL domain, network proxy, and DNS configuration.',
  tls_error: 'TLS/certificate handshake failed; check HTTPS certificate, transit gateway, or corporate proxy policy.',
  connection_refused: 'Target service refused connection; verify Base URL port, service process, and firewall configuration.',
  model_access_denied: 'Current account cannot use this model; confirm whether the model is enabled, the account has visibility, or the model has been disabled.',
  provider_prefix_mismatch: 'Model provider prefix does not match the current channel; verify whether the model name should use this channel\'s OpenAI-compatible route.',
  capability_unsupported: 'Current model or compatibility layer does not support this capability; this does not affect basic text connectivity, you can switch models or disable this capability dependency.',
};
...
function getLlmTroubleshootingHint(
  ...
  if (code === 'format_error') {
    return context === 'discovery' || stage === 'model_discovery'
      ? 'The /models response format from this channel is incompatible; please fill in the model list manually.'
      : 'Response structure does not match expectations; verify that this channel is compatible with the Chat Completions API.';
  }
  if (code === 'empty_response' && (context === 'discovery' || stage === 'model_discovery')) {
    return 'The /models endpoint of this channel returned no available model IDs; check if the Base URL points to a compatible model list endpoint, or fill in the model list manually.';
  }
...
function buildLlmTestHint(result: {
  ...
  const scopeInfo = 'Basic connection test uses the first model in the list by default.';
  ...
  const modelActionHint = shouldSuggestModelListChange
    ? 'If this model is unavailable, adjust the model order or remove unavailable models and retry.'
    : '';
...
function buildLlmFailureText(result: {
  ...
  if (result.error && result.error !== result.message) {
    return `${prefix}: ${summary} (raw: ${result.error})`;
  }
...
export const LLMChannelEditor: React.FC<LLMChannelEditorProps> = ({
  ...
  const handleSave = async () => {
    ...
    if (hasEmptyName) {
      setSaveMessage({ type: 'local-error', text: 'Channel name cannot be empty and may only contain letters, numbers, or underscores.' });
      return;
    }
    ...
    if (managesRuntimeConfig) {
      ...
      if (mixedPrimary || mixedFallback) {
        setSaveMessage({ type: 'local-error', text: 'Mixed Hermes/non-Hermes routes are not supported as primary or fallback models. Please select pure Hermes or pure non-Hermes routes.' });
        return;
      }
      ...
      if (nonCanonicalRouteAlias) {
        setSaveMessage({ type: 'local-error', text: 'Runtime models use non-canonical route aliases. Please re-select from the dropdown.' });
        return;
      }
    }
    ...
    if (managesRuntimeConfig) {
      ...
      if (invalidPrimaryModel) {
        setSaveMessage({ type: 'local-error', text: 'Primary model is not in any enabled channel model list. Please re-select.' });
        return;
      }
      ...
      if (invalidAgentPrimaryModel) {
        setSaveMessage({ type: 'local-error', text: 'Agent primary model has no Agent-safe non-Hermes deployment. Please re-select.' });
        return;
      }
      ...
      if (invalidFallbackModel) {
        setSaveMessage({ type: 'local-error', text: 'Invalid fallback models detected. Please re-select.' });
        return;
      }
      ...
      if (invalidVisionModel) {
        setSaveMessage({ type: 'local-error', text: 'Vision model cannot include Hermes deployments. Please select a pure non-Hermes route.' });
        return;
      }
    }
      ...
      >
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-text">
            After adding provider channels, models can be auto-discovered and multi-selected, or you can continue to fill in manually. Configuration syncs automatically to the .env file.
          </p>
        </div>
      ...
      {!isCollapsed ? (
        <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="rounded-[1.35rem] border border-[var(--settings-border)] bg-[var(--settings-surface)] p-4 shadow-soft-card">
            <div className="mb-3 flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-xs text-secondary-text">Select a provider preset, then create a configuration draft with one click.</p>
              </div>
          ...
          <div className="space-y-2">
            ...
            {channels.length === 0 ? (
              <div className="settings-surface-overlay-muted rounded-[1.35rem] border border-dashed settings-border-strong px-4 py-10 text-center">
                ...
                <p className="mt-1 text-xs text-muted-text">Select a provider preset and click 'Add Channel' to get started.</p>
              </div>
          ...
          {managesRuntimeConfig ? (
            <div className="rounded-[1.35rem] border border-[var(--settings-border)] bg-[var(--settings-surface)] p-4 shadow-soft-card">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-[11px] text-muted-text">Primary model, fallback models, Vision, and Temperature are written directly to the runtime configuration.</p>
                </div>
              ...
              <div className="mb-4">
                ...
                <p className="mt-1 text-[11px] text-secondary-text">
                  Controls output randomness; 0 is deterministic, 2 is maximum randomness. Recommended: 0.7.
                </p>
              </div>
              ...
              {availableModels.length === 0 ? (
                <div className="rounded-xl border border-dashed settings-border-strong settings-surface-overlay-soft px-3 py-2 text-xs text-muted-text">
                  Add at least one enabled channel with models configured, and the primary / fallback / Vision options below will appear.
                </div>
              ) : (
                <div className="space-y-4">
                  <div>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      options={buildModelOptions(
                        ...
                        'Auto (inherit from primary model)',
                      )}
                      disabled={busy}
                      placeholder=""
                    />
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-[11px] text-secondary-text">
                      Fallback models are only used when the primary model fails. The primary model is automatically excluded from fallback options.
                    </p>
                  </div>
            ...
            <InlineAlert
              variant="warning"
              message="Advanced model routing YAML detected: this section only manages channel entries and basic connection info. Runtime primary / fallback / Vision / Temperature are still controlled by the general fields below; if YAML parsing succeeds, its routing and available model declarations take precedence, and this configuration will not override the YAML file itself."
