...
describe('AnalysisContextSummary', () => {
  ...
  it('surfaces degraded non-zero states in the collapsed summary', () => {
    ...
    expect(within(quoteBlock as HTMLElement).getByText('说明: 本次分析使用了备用数据路径；请结合来源 and 告警复核结果')).toBeInTheDocument();
    ...
    expect(screen.getByText('说明: 仅部分数据进入本次分析，相关结论可能不完整；请检查告警并数据源后重新分析')).toBeInTheDocument();
