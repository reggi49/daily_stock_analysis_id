...
test.describe('ReportMarkdown component', () => {
  test('copy markdown source code', async ({ page, context }) => {
    ...
    const detailedReportButton = page.getByRole('button', { name: 'Full Analysis Report' });
    await expect(detailedReportButton).toBeEnabled({ timeout: 3000 });

    // Click the "Full Analysis Report" button to open the markdown drawer
    ...
    await expect(page.getByRole('dialog').getByText('Full Analysis Report')).toBeVisible();

    // Click copy markdown button
    const copyMarkdownButton = page.getByRole('button', { name: 'Copy Markdown Source' });
    ...
    const checkmarkIcon = page.locator('button[aria-label="Copy Markdown Source"] svg.text-success');
  ...
  test('copy plain text', async ({ page, context }) => {
    ...
    const copyPlainTextButton = page.getByRole('button', { name: 'Copy Plain Text' });
    ...
    // Verify checkmark icon is shown
    const checkmarkIcon = page.locator('button[aria-label="Copy Plain Text"] svg.text-success');
    await expect(checkmarkIcon).toBeVisible();
  ...
  test('mobile responsive layout', async ({ page }) => {
    ...
    await expect(page.getByPlaceholder('Enter stock code or name, e.g. 600519, 贵州茅台, AAPL')).toBeVisible({ timeout: 10_000 });
    ...
    await expect(page.getByRole('dialog').getByText('Full Analysis Report')).toBeVisible({ timeout: 10000 });
