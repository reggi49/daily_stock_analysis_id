...
def _write_stock_index(path: Path, name: str = "Ping An Bank", size: int = 1) -> None:
...
def test_stock_index_route_serves_newer_remote_cache(tmp_path: Path) -> None:
    ...
    _write_stock_index(static_dir / "stocks.index.json", "Built-in static")
    _write_stock_index(cache_path, "remote cache", size=100)
    _write_stock_index(bundled_path, "Built-in source code")
    ...
    assert response.json()[0][2] == "remote cache"
...
def test_stock_index_route_prefers_newer_static_index_over_older_remote_cache(tmp_path: Path) -> None:
    ...
    cache_path = tmp_path / "cache" / "stocks.index.json"
    bundled_path = tmp_path / "bundled" / "stocks.index.json"
    _write_stock_index(static_dir / "stocks.index.json", "New built-in static")
    _write_stock_index(cache_path, "old remote cache", size=100)
    _write_stock_index(bundled_path, "Built-in source code")
    ...
    assert response.status_code == 200
    assert response.json()[0][2] == "New built-in static"
...
def test_stock_index_route_falls_back_to_static_index(tmp_path: Path) -> None:
    ...
    assert response.status_code == 200
    assert response.json()[0][2] == "Built-in static"
