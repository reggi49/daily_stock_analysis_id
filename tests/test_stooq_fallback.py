...
class TestStooqFallback(unittest.TestCase):
    ...
    def test_stooq_success_logic(self, mock_urlopen):
        """Test Stooq normal fetch and parse logic"""
        # Mock the CSV-format data returned by Stooq (realtime + daily history)
    ...
    def test_fetcher_integration_with_fallback(self, mock_ticker_class):
        """Test that Stooq logic is auto-triggered after yfinance fails"""
        # 1. Simulate yfinance fully failing
        mock_ticker = MagicMock()
        # Simulate fast_info attribute access raising
        type(mock_ticker).fast_info = PropertyMock(side_effect=Exception("API Error"))
        # Simulate history returning empty
        mock_ticker.history.return_value = MagicMock(empty=True)
        mock_ticker_class.return_value = mock_ticker

        # 2. Simulate Stooq returning successfully
