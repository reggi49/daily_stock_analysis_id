...
def test_search_service_init_failure_logs_traceback_and_failure_state(caplog):
    ...
    init_failure_records = [
        record for record in caplog.records if "Search service initialization failed，will run in searchless mode" in record.message
    ]
    assert len(init_failure_records) == 1
    assert init_failure_records[0].exc_info is not None
    assert "Search service is not enabled（Initialization failed or dependencies are missing）" in caplog.text
    assert "Search service is not enabled（Search capability not configured）" not in caplog.text
...
def test_social_sentiment_init_failure_logs_traceback(caplog):
    ...
    init_failure_records = [
        record for record in caplog.records if "Social public opinion service initialization failed，Public opinion analysis will be skipped" in record.message
    ]
    assert len(init_failure_records) == 1
    assert init_failure_records[0].exc_info is not None
