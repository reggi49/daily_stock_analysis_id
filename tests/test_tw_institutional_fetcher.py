...
T86_FIXTURE = {
    ...
    "date": "20260626",
    "fields": [
        "Securities code", "Security name",
        "Number of shares purchased by overseas investors(Excluding foreign-owned operators)", "Number of shares sold by overseas investors(Excluding foreign-owned operators)", "Overseas shares traded by mainland investors(Excluding foreign-owned operators)",
        "Number of shares purchased by foreign self-operators", "Number of shares sold by foreign self-operators", "Foreign self-operated dealers buy and sell excess shares",
        "Number of shares purchased by Investment Trust", "Number of shares sold by Investment Trust", "Investment trust trading exceeds the number of shares",
        "Dealers buy and sell excess shares",
        "Number of shares purchased by proprietary traders(Buy and sell on your own)", "Number of shares sold by proprietary traders(Buy and sell on your own)", "Dealers buy and sell excess shares(Buy and sell on your own)",
        "Number of shares purchased by proprietary traders(Hedging)", "Number of shares sold by proprietary traders(Hedging)", "Dealers buy and sell excess shares(Hedging)",
        "The three major legal persons bought and sold excess shares",
    ],
    "data": [
        ["2330", "TSMC          ", "22,676,018", "36,957,173", "-14,281,155",
         "0", "0", "0", "1,034,258", "299,860", "734,398",
         "1,009,368", "226,100", "769,604", "-543,504",
         "3,424,484", "1,871,612", "1,552,872", "-12,537,389"],
        ["2337", "Macronix            ", "99,038,413", "41,711,072", "57,327,341",
...
class TestPureHelpers(unittest.TestCase):
    def test_minguo_to_ad(self):
        self.assertEqual(minguo_to_ad("1150626"), "20260626")  # Republic of China115 -> AD2026
        self.assertEqual(minguo_to_ad("1010101"), "20120101")  # Republic of China101 -> AD2012
        self.assertEqual(minguo_to_ad("0010101"), "19120101")  # Republic of China1   -> AD1912
        self.assertEqual(minguo_to_ad("0990101"), "20100101")  # Republic of China99  -> AD2010
...
class TestT86Parsing(unittest.TestCase):
    def test_twse_2330_net_breakdown(self):
        ...
        self.assertEqual(rec["market"], "Listed")
    ...
    def test_twse_stat_not_ok_fails_open(self):
        bad = {"stat": "sorry，No matching data!", "data": []}
...
class TestTpexParsing(unittest.TestCase):
    def test_tpex_3105_net_breakdown_and_minguo_date(self):
        ...
        self.assertEqual(rec["market"], "On the counter")
        self.assertEqual(rec["source"], "TPEx-OpenAPI")
        self.assertEqual(rec["date"], "20260626")        # Republic of China 1150626 -> AD
...
class TestMissingFieldAndEmptyCacheFailOpen(unittest.TestCase):
    ...
    def test_tpex_genuine_zero_is_kept(self):
        import copy
        row = copy.deepcopy(TPEX_FIXTURE[0])
        row["SecuritiesInvestmentTrustCompanies-Difference"] = "0"  # Put a letter truly net-zero today
        with patch("data_provider.tw_institutional_fetcher.requests.get", return_value=_resp([row])):
            rec = _fetcher().get_institutional_net("3105.TWO")
        self.assertIsNotNone(rec)
    ...
    def test_twse_missing_core_field_drops_row(self):
        ...
        trust_idx = fix["fields"].index("Investment trust trading exceeds the number of shares")
        fix["data"][0][trust_idx] = ""   # Investment trust trading super cell blank -> missing -> drop 2330, not 0
...
class TestStructureRobustness(unittest.TestCase):
    ...
    def test_twse_renamed_core_field_fails_open(self):
        import copy
        fix = copy.deepcopy(T86_FIXTURE)
        fix["fields"][10] = "Investment trust trading exceeds the number of shares_v2"          # Put a letter column renamed
        with patch("data_provider.tw_institutional_fetcher.requests.get", return_value=_resp(fix)):
            self.assertIsNone(_fetcher().get_institutional_net("2330.TW", "20260626"))
    ...
    def test_tpex_unconvertible_date_drops_row(self):
        import copy
        row = copy.deepcopy(TPEX_FIXTURE[0])
        row["Date"] = "bad-date"                        # not a 7-digit Republic of China -> drop
        with patch("data_provider.tw_institutional_fetcher.requests.get", return_value=_resp([row])):
            self.assertIsNone(_fetcher().get_institutional_net("3105.TWO"))
