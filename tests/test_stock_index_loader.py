...
class TestStockIndexLoader(unittest.TestCase):
    ...
    def test_get_index_stock_name_supports_display_canonical_and_hk_keys(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            index_path.write_text(
                json.dumps(
                    [
                        ["000001.SZ", "000001", "Ping An Bank", "pinganyinhang", "payh", [], "CN", "stock", True, 100],
                        ["00700.HK", "00700", "Tencent Holdings", "tengxunkonggu", "txkg", [], "HK", "stock", True, 100],
                        ["AAPL", "AAPL", "apple", "pingguo", "pg", [], "US", "stock", True, 100],
                    ],
                    ensure_ascii=False,
                ),
                encoding="utf-8",
            ...
            with patch.object(stock_index_loader, "get_stock_index_candidate_paths", return_value=(index_path,)):
                self.assertEqual(stock_index_loader.get_index_stock_name("000001"), "Ping An Bank")
                self.assertEqual(stock_index_loader.get_index_stock_name("000001.SZ"), "Ping An Bank")
                self.assertEqual(stock_index_loader.get_index_stock_name("HK00700"), "Tencent Holdings")
                self.assertEqual(stock_index_loader.get_index_stock_name("00700"), "Tencent Holdings")
                self.assertEqual(stock_index_loader.get_index_stock_name("700.HK"), "Tencent Holdings")
                self.assertEqual(stock_index_loader.get_index_stock_name("aapl"), "apple")
    ...
    def test_get_stock_name_index_map_is_cached_after_first_load(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            index_path = Path(temp_dir) / "stocks.index.json"
            index_path.write_text(
                json.dumps([["000001.SZ", "000001", "Ping An Bank"]], ensure_ascii=False),
                encoding="utf-8",
            )
            ...
            with patch.object(stock_index_loader, "get_stock_index_candidate_paths", return_value=(index_path,)):
                ...
                index_path.write_text(
                    json.dumps([["000001.SZ", "000001", "Name after change"]], ensure_ascii=False),
                    encoding="utf-8",
                )
    ...
    def test_get_stock_name_index_map_skips_unexpected_json_shape_and_uses_next_candidate(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            malformed_path.write_text(
                json.dumps({"code": "000001", "name": "Ping An Bank"}, ensure_ascii=False),
                encoding="utf-8",
            )
            valid_path.write_text(
                json.dumps([["000001.SZ", "000001", "Ping An Bank"]], ensure_ascii=False),
    ...
    def test_newer_bundled_index_wins_over_older_remote_cache(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            _write_stock_index(remote_cache, "old remote cache", size=100)
            _write_stock_index(bundled_path, "New built-in index")
            ...
            with patch.object(stock_index_loader, "get_remote_stock_index_cache_path", return_value=remote_cache), \
                ...
                self.assertEqual(stock_index_loader.get_index_stock_name("000001"), "New built-in index")

    def test_newer_remote_cache_wins_when_valid(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            remote_cache = Path(temp_dir) / "cache" / "stocks.index.json"
            bundled_path = Path(temp_dir) / "apps" / "stocks.index.json"
            _write_stock_index(remote_cache, "New remote cache", size=100)
            _write_stock_index(bundled_path, "old built-in index")
            ...
            with patch.object(stock_index_loader, "get_remote_stock_index_cache_path", return_value=remote_cache), \
                ...
                self.assertEqual(stock_index_loader.get_index_stock_name("000001"), "New remote cache")
    ...
    def test_invalid_remote_cache_is_skipped_even_when_newer(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            _write_stock_index(bundled_path, "built-in index")
            os.utime(remote_cache, (2_000, 2_000))
            os.utime(bundled_path, (1_000, 1_000))
            ...
            with patch.object(stock_index_loader, "get_remote_stock_index_cache_path", return_value=remote_cache), \
                ...
                self.assertEqual(stock_index_loader.find_existing_stock_index_path(), bundled_path)
                self.assertEqual(stock_index_loader.get_index_stock_name("000001"), "built-in index")
    ...
    def test_resolve_index_stock_code_does_not_bare_resolve_tw_suffix_entries(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            bundled_path.write_text(
                json.dumps(
                    [
                        ["2330.TW", "2330.TW", "TSMC", "taijidian", "tjd", [], "TW", "stock", True, 100],
                        ["6505.TWO", "6505.TWO", "Formosa Plastics", "taisu", "ts", [], "TW", "stock", True, 100],
                    ],
                    ensure_ascii=False,
                ),
                encoding="utf-8",
