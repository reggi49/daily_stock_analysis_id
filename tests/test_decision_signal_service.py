...
def _payload(**overrides):
    payload = {
        ...
        "score": 83,
        "horizon": "3d",
        "reason": "Heavy volume breakthrough",
    }
...
def _history_result(**overrides):
    ...
    result = AnalysisResult(
        ...
        sentiment_score=68,
        trend_prediction="The shock is strong",
        operation_advice="hold observation",
        decision_type="hold",
        confidence_level="in",
        analysis_summary="The trend is still，But wait for the amount to be confirmed。",
        report_language="zh",
    )
    result.dashboard = {
        "battle_plan": {
            ...
            "action_checklist": ["Step back without breaking the support"],
        }
    }
...
def test_list_signals_lazily_backfills_analysis_history_signal(isolated_db) -> None:
    ...
    assert item["action_label"] == "hold"
    assert item["reason"] == "The trend is still，But wait for the amount to be confirmed。"
    assert item["watch_conditions"] == '["Step back without breaking the support"]'
...
def test_list_signals_invalidates_stale_backfill_when_newer_opposing_signal_exists(isolated_db) -> None:
    record_id = isolated_db.save_analysis_history(
        result=_history_result(
            ...
            action="buy",
            action_label="Buy",
            analysis_summary="Old report recommends buying。",
        ),
...
def test_list_signals_does_not_backfill_market_review_history(isolated_db) -> None:
    record_id = isolated_db.save_analysis_history(
        result=_history_result(code="MARKET", name="Market review", operation_advice="View review"),
...
def test_list_signals_does_not_backfill_ambiguous_history_default_decision_type_hold_with_noisy_advice(
    ...
    record_id = isolated_db.save_analysis_history(
        result=_history_result(
            operation_advice="Buying orders strengthen，Continue to observe",
            decision_type="hold",
            action=None,
            action_label=None,
        ),
...
def test_list_signals_explicit_stock_identities_do_not_trigger_history_backfill(isolated_db) -> None:
    record_id = isolated_db.save_analysis_history(
        result=_history_result(operation_advice="sell", decision_type="sell", action="sell", action_label="sell"),
