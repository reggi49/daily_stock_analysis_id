    ...
    def test_parse_skill_matches_alias_before_default(self) -> None:
        ...
        skills = [
            ...
            Skill(
                ...
                description="chan",
                instructions="chan",
                aliases=["entanglement", "entanglement analysis"],
                default_priority=40,
            ),
        ]

        with patch.object(AskCommand, "_load_skills", return_value=skills):
            self.assertEqual(command._parse_skill(["600519", "please", "Analysis using entanglement theory"]), "chan_theory")
...
class TestAskCommandMultiStock(unittest.TestCase):
    ...
    def test_analyze_multi_includes_portfolio_overlay(self):
        ...
        with patch("src.agent.factory.build_agent_executor", return_value=FakeExecutor()):
            with patch.object(command, "_build_portfolio_section", return_value="## combined perspective\nportfolio summary"):
        ...
        self.assertIn("## combined perspective", response.text)
    ...
    def test_merge_code_args_keeps_comma_split_multi_stock_support(self):
        ...
        raw_code_str, remaining_args = command._merge_code_args(["600519,", "000858", "wave theory"])

        self.assertEqual(raw_code_str, "600519,000858")
        self.assertEqual(remaining_args, ["wave theory"])
    ...
    def test_build_portfolio_section_reads_assessment(self):
        ...
        results = {
            "600519": {
                "signal": "buy",
                "confidence": 0.8,
                "summary": "Moutai summary",
                "stock_name": "Kweichow Moutai",
                "risk_flags": [{"category": "portfolio_input", "description": "Valuation is high", "severity": "medium"}],
            },
            "000858": {
                "signal": "hold",
                "confidence": 0.6,
        ...
        def fake_run(self, ctx, progress_callback=None):
            ctx.data["portfolio_assessment"] = {
                "summary": "The portfolio is more consumption-focused，It is recommended to control the position。",
                "portfolio_risk_score": 7,
                "sector_warnings": ["The liquor sector is too concentrated"],
                "correlation_warnings": ["600519 with 000858 High correlation"],
                "rebalance_suggestions": ["Reduce single industry exposure"],
                "positions": [
        ...
        self.assertIn("## combined perspective", text)
        self.assertIn("The portfolio is more consumption-focused", text)
    ...
    def test_analyze_multi_falls_back_to_text_when_dashboard_parse_fails(self):
        ...
        class FakeExecutor:
            def run(self, task, context=None):
                ...
                return SimpleNamespace(
                    success=False,
                    content=f"{code} free text analysis",
                    dashboard=None,
        ...
        self.assertIn("600519 free text analysis", response.text)
    ...
    def test_analyze_multi_persists_formatted_history_instead_of_raw_json(self):
        ...
        self.assertTrue(all("**Conclusion**: buy" in text for text in assistant_messages))
    ...
    def test_analyze_single_passes_requested_skill_into_context(self):
        ...
        with patch("src.agent.factory.build_agent_executor", return_value=FakeExecutor()):
            with patch.object(command, "_resolve_skill_name", return_value="entanglement"):
