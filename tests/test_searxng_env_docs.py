...
def test_env_example_documents_searxng_actions_variable_mapping() -> None:
    ...
    assert "Variables Priority" in searxng_block
    assert "Secrets" in searxng_block
    assert "Need to be configured as Secret" not in searxng_block
...
def test_changelog_mentions_searxng_actions_variable_mapping() -> None:
    ...
    assert (
        "- [Repair] GitHub Actions Daily analysis workflow reads SearXNG When creating a self-created instance address"
        "support Variables Priority、Secrets rollback，Fix configuration only Variables time URL Invalid problem。"
    ) in changelog
