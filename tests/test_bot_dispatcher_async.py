...
class TestCommandDispatcherAsync(unittest.IsolatedAsyncioTestCase):
    def test_nl_prefilter_matches_bse_codes(self):
        self.assertIsNotNone(CommandDispatcher._NL_PREFILTER.search("Help me analyze430001"))
    ...
    async def test_parse_intent_via_llm_offloads_to_thread(self):
        ...
        with patch(
            ...
            with patch("src.agent.llm_adapter.LLMToolAdapter") as adapter_cls:
                ...
                result = await CommandDispatcher._parse_intent_via_llm("analysis600519", config)
    ...
    async def test_try_nl_routing_uses_async_command_execution(self):
        ...
        with patch("src.config.get_config", return_value=config):
            with patch.object(dispatcher, "_parse_intent_via_llm", new=AsyncMock(return_value={
                ...
                result = await dispatcher._try_nl_routing(_make_message("Help me analyze600519", mentioned=True))
        ...
        with patch("src.config.get_config", return_value=config):
            with patch(
                ...
                with patch.object(
                    ...
                    result = await dispatcher._try_nl_routing(_make_message("Help me analyze Moutai", mentioned=True))
