...
class _FallbackNameFetcher:
    ...
    def __init__(self, name: str = "alternative name"):
...
class TestPrefetchStockNames(unittest.TestCase):
    ...
    def test_get_stock_name_skips_realtime_when_allow_realtime_false(self):
        ...
        manager.get_realtime_quote = MagicMock(return_value=MagicMock(name="live name"))

        name = DataFetcherManager.get_stock_name(manager, "123456", allow_realtime=False)

        self.assertEqual(name, "test stocks")
    ...
    def test_get_stock_name_prefers_index_mapping_before_remote_fetchers(self):
        ...
        with patch("data_provider.base.get_index_stock_name", return_value="Index name"):
            name = DataFetcherManager.get_stock_name(manager, "123456", allow_realtime=False)

        self.assertEqual(name, "Index name")
        manager.get_realtime_quote.assert_not_called()
        remote_fetcher.get_stock_name.assert_not_called()
        self.assertEqual(manager._stock_name_cache["123456"], "Index name")
    ...
    def test_get_stock_name_prefers_static_mapping_before_index_hits(self):
        ...
        with patch.dict("data_provider.base.STOCK_NAME_MAP", {"AAPL": "apple"}, clear=True):
    ...
    def test_get_stock_name_prefers_index_mapping_before_realtime_quote(self):
        manager = DataFetcherManager.__new__(DataFetcherManager)
        manager._fetchers = []
        manager.get_realtime_quote = MagicMock(return_value=SimpleNamespace(name="live name"))
    ...
    def test_get_stock_name_preserves_raw_exchange_hint_for_realtime_lookup(self):
        manager = DataFetcherManager.__new__(DataFetcherManager)
        manager._fetchers = []
        manager.get_realtime_quote = MagicMock(return_value=SimpleNamespace(name="Ping An Bank"))
    ...
    def test_pytdx_get_stock_name_reads_all_security_list_pages(self):
        ...
        first_page = [
            {"code": f"{index:06d}", "name": f"stocks{index:06d}"}
            for index in range(1000)
        ]
        second_page = [{"code": "300750", "name": "Ningde era"}]
        ...
        self.assertEqual(name, "Ningde era")
        self.assertEqual(fetcher._stock_name_cache["300750"], "Ningde era")
        self.assertEqual(fetcher._stock_list_cache["300750"], "Ningde era")
    ...
    def test_manager_skips_pytdx_name_lookup_during_connection_cooldown(self):
        ...
        fallback = _FallbackNameFetcher("GEM Artificial IntelligenceETF")
        ...
        self.assertEqual(first, "GEM Artificial IntelligenceETF")
        self.assertEqual(second, "GEM Artificial IntelligenceETF")
    ...
    def test_batch_get_stock_names_serializes_shared_fetcher_access(self):
        ...
        for result in results:
            self.assertEqual(result["600519"], "Kweichow Moutai")
            self.assertEqual(result["000001"], "Ping An Bank")
        self.assertGreaterEqual(manager._fetchers[0].call_count, 1)
        self.assertEqual(manager._stock_name_cache["600519"], "Kweichow Moutai")
        self.assertEqual(manager._stock_name_cache["000001"], "Ping An Bank")
