...
Usage:
    ...
    /ask 600519 analyze using Chan Theory -> Parse skill from message
    /ask 600519 chan_theory             -> Specify skill id directly
    /ask 600519,000858 Elliott Wave    -> Multi-stock comparison with skill overlay
"""
...
class AskCommand(BotCommand):
    ...
    def execute(self, message: BotMessage, args: List[str]) -> BotResponse:
        ...
        if not config.agent_mode:
            return BotResponse.text_response(
                "⚠️ Agent mode is not enabled, stock query is unavailable.\nPlease set `AGENT_MODE=true` in your config."
            )
    ...
    def _analyze_single(
        ...
        try:
            ...
            return BotResponse.text_response(f"⚠️ Stock query error: {str(exc)}")
        ...
        try:
            for future in as_completed(future_map, timeout=overall_timeout_s):
                try:
                    ...
                    errors[code] = f"Execution exception: {exc}"
    ...
    def _format_stock_result(stock_code: str, dashboard: Optional[Dict[str, Any]], raw_content: str) -> str:
        if not isinstance(dashboard, dict):
            content = raw_content
            if len(content) > 800:
                content = content[:800] + "\n... (truncated, query individually for full analysis)"
            return content
    ...
    def _build_portfolio_section(
        ...
        def _render_overlay() -> str:
            ...
            if rebalance:
                lines.append(f"- Rebalance suggestions: {'; '.join(str(item) for item in rebalance[:3])}")
