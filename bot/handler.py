...
Handles Webhook callbacks from various platforms and dispatches them to command handlers.
"""
...
def handle_webhook(
    ...
    Args:
        ...
        query_params: URL query parameters (used for some platform verifications)
    ...
    # Check if bot feature is enabled
    from src.config import get_config
    config = get_config()

    if not getattr(config, 'bot_enabled', True):
        logger.info("[BotHandler] Bot feature is not enabled")
    ...
    # If it's a verification/error response with no message to process, return immediately
    if immediate_response and not message:
        logger.info("[BotHandler] Returning verification response")
        return immediate_response

    # Deferred response (e.g. Discord type 5): return ACK immediately, process command in background
    ...
    # If no message to process, return empty response
    if not message:
...
def handle_wecom_webhook(headers: Dict[str, str], body: bytes) -> WebhookResponse:
    """Handle WeCom (Enterprise WeChat) Webhook"""
